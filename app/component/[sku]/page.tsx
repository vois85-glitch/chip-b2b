...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${component.sku} — купить, цена, аналоги, даташит | ChipNet`,
    description: `Поставка ${component.name} (${component.brand}). ${component.status === 'EOL' ? 'Снят с производства. Подберем аналог.' : 'В наличии под заказ.'} Проверка в лаборатории СВП. ООО Деловой Партнёр.`,
  };
}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2">
            ...
            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold mb-8 ${
              ...
              {component.status === 'EOL' ? '⚠️ Снят с производства (EOL)' : '✅ В производстве'}
            </div>
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-24 bg-emerald-950/10 border border-emerald-900/30 rounded-2xl p-8">
              ...
              <a 
                href="/#bom"
                className="block w-full text-center px-8 py-4 bg-emerald-600 hover:bg-emerald-700 rounded-xl text-lg font-semibold transition-all shadow-lg shadow-emerald-600/25 hover:shadow-emerald-600/40 mb-4"
              >
                Отправить заявку
              </a>
