  ...
  const filterSidebar = (
    <>
      ...
      <div className="bg-[#fafafa] border border-[#e8e8e8] rounded-xl p-4 mb-4">
        <h3 className="text-sm font-bold text-[#333] mb-3 uppercase tracking-wider">Категории</h3>
        <div className="relative mb-3">
          <input
            ...
            className="w-full px-3 py-2 bg-white border border-[#e8e8e8] rounded-lg text-sm text-[#121212] placeholder-[#999] focus:outline-none focus:border-primary"
          />
          {catSearch && (
            ...
            <button
              ...
              className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex justify-between items-center ${
                currentCategory === cat
                  ...
                  : 'text-[#666] hover:text-[#121212] hover:bg-[#f3f3f3]'
              }`}
            >
      ...
      <div className="bg-[#fafafa] border border-[#e8e8e8] rounded-xl p-4">
        <h3 className="text-sm font-bold text-[#333] mb-3 uppercase tracking-wider">Бренды</h3>
        <div className="relative mb-3">
          <input
            type="text"
            value={brandSearch}
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <form onSubmit={handleSearch} className="flex gap-2 sm:gap-3">
            <div className="flex-1 relative">
              <input
                ...
                className="w-full px-4 sm:px-5 py-3 sm:py-3.5 bg-white border border-[#e8e8e8] rounded-xl text-[#121212] placeholder-[#999] focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/30 transition-all text-sm sm:text-base"
              />
              {searchInput && (
                <button
                  type="button"
            ...
            <button
              type="submit"
              className="px-4 sm:px-6 py-3 sm:py-3.5 bg-primary hover:bg-primary-dark rounded-xl font-semibold text-[#121212] transition-colors text-sm sm:text-base"
            >
              Найти
            </button>
            <button
              ...
              className={`relative px-3 sm:px-4 py-3 sm:py-3.5 border rounded-xl font-semibold transition-colors text-sm sm:text-base ${
                showFilters
                  ...
                  : 'border-[#e8e8e8] bg-white text-[#666] hover:text-[#121212]'
              }`}
            >
              ...
              {activeFilterCount > 0 && (
                <span className="absolute -top-2 -right-2 w-5 h-5 flex items-center justify-center bg-emerald-500 text-[#121212] text-[10px] font-bold rounded-full">
                  {activeFilterCount}
                </span>
              )}
            </button>
        ...
        {hasFilters && (
          <div className="flex flex-wrap items-center gap-2 mb-4">
            <span className="text-[#757575] text-sm">Фильтры:</span>
            {currentCategory && (
            ...
            {components.length > 0 ? (
              <div className={`grid gap-3 ${showFilters ? 'grid-cols-1 sm:grid-cols-2 xl:grid-cols-3' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3'}`}>
                {components.map((comp) => (
                  <Link
                    ...
                    className="group block bg-gray-900/40 border border-[#e8e8e8] rounded-xl p-4 hover:bg-gray-900/70 hover:border-emerald-800/50 transition-all duration-200"
                  >
                    <div className="flex items-start justify-between mb-2">
                      ...
                      <span className={`flex-shrink-0 ml-2 px-2 py-0.5 rounded text-[10px] font-semibold ${
                        comp.status === 'EOL'
                          ...
                          : 'bg-section-alt text-primary'
                      }`}>
                        {comp.status === 'EOL' ? 'EOL' : 'В пр.'}
                      </span>
                    </div>
                    <p className="text-[#666] text-xs leading-relaxed mb-2 line-clamp-2">
                      {comp.name}
                    </p>
                    <div className="flex items-center gap-2">
                      <span className="px-2 py-0.5 bg-gray-800/80 rounded text-[10px] text-[#757575]">
                        {comp.brand}
                      </span>
                      <span className="px-2 py-0.5 bg-gray-800/80 rounded text-[10px] text-[#757575]">
                        {comp.category}
              ...
              <div className="text-center py-20">
                ...
                <button
                  ...
                  className="px-6 py-2.5 bg-primary hover:bg-primary-dark rounded-lg font-semibold text-[#121212] transition-colors"
                >
                  Сбросить фильтры
                </button>
              </div>
              ...
              <div className="flex items-center justify-center gap-2 mt-8">
                {currentPage > 1 && (
                  <Link
                    ...
                    className="px-4 py-2 bg-white border border-[#e8e8e8] rounded-lg text-[#666] hover:text-[#121212] hover:border-emerald-600/50 transition-colors"
                  >
                    ← Назад
                  </Link>
                )}
                  ...
                  <Link
                    ...
                    className={`w-10 h-10 flex items-center justify-center rounded-lg font-medium transition-colors ${
                      pageNum === currentPage
                        ...
                        : 'bg-white border border-[#e8e8e8] text-[#666] hover:text-[#121212] hover:border-emerald-600/50'
                    }`}
                  >
                    {pageNum}
                  </Link>
        ...
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-[85vw] max-w-sm bg-[#0a0d0c] border-r border-[#e8e8e8] flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#e8e8e8]">
              ...
              <button
                onClick={() => setShowFilters(false)}
                className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#f3f3f3] text-[#666] hover:text-[#121212] transition-colors"
              >
            ...
            <div className="px-5 py-4 border-t border-[#e8e8e8] flex gap-3">
              {hasFilters && (
                <button
                  onClick={clearFilters}
                  className="flex-1 py-3 border border-[#e8e8e8] rounded-xl text-[#666] hover:text-[#121212] hover:border-gray-600 transition-colors text-sm font-semibold"
                >
              ...
              <button
                onClick={() => setShowFilters(false)}
                className="flex-1 py-3 bg-primary hover:bg-primary-dark rounded-xl text-[#121212] text-sm font-semibold transition-colors"
              >
                Показать результаты
              </button>
            </div>
